...
Чтобы система вызывала `onNewToken`, добавь в **`src/main/AndroidManifest.xml`** внутрь `<application>` (только пока используешь локальный `google-services.json`):

```xml
<service
    android:name=".HiAirFirebaseMessagingService"
...
**Важно:** не коммить этот блок без JSON — сборка упадёт, если класс не компилируется. Держи правку локально или в приватной ветке вместе с `google-services.json`.

Без этого сервиса токен всё равно подтягивается при вызове `FcmFirebaseBootstrap.refresh` (логин / `PushTokenRegistrar`).
